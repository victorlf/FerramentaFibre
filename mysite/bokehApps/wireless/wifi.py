...
def print_table(table):
    ...
    length = len(justified_table)
    for line in justified_table[1:length]:
        if line[0].startswith('BNDVIS'):   #melhor do que comparar com uma string, pois em algum momento espacos sao
            for el in line:             #acrescentados na string
                print (el),
            print()
...
def print_cells_on_table(cells, tempo):
    table = [columns]
    for cell in cells:
        cell_properties = []
        for column in columns:
            cell_properties.append(cell[column])
        table.append(cell_properties)
    return insert_table(table, tempo)


def insert_table(table, tempo):
    widths = map(max, map(lambda l: map(len, l), zip(*table)))  # functional magic
    justified_table = []
    for line in table:
        justified_line = []
    ...
    for line[:length] in justified_table[1:length]:
        param = list()
        if line[0].startswith('BNDVIS'):               #melhor do que comparar com uma string, pois em algum momento espacos sao
            line[3] = line[3].replace(' dBm  ',"")  #acrescentados na string
            #s = Signal(name=line[0], adress=line[1], quality=line[2], signal=line[3], channel=line[4], encryption=line[5], time=tempo)
            #s.save()
            #return line[3]
            param.append(line[3])
...
def main():
    ...
    while True:
        ...
        return print_cells_on_table(parsed_cells, tempo)
