...
from UFGrunExp import run, reWriteExp, insereTabela
...
def update_data():
    ...
    #params = run(nodes, transf, size)
    #reWriteExp("wireless_experiment_4", nodes, transf, size)
    insereTabela("wireless_experiment_3")
    ...
    # for line in params:
    #     if int(line[1]) == 1:
    #         temp = int(line[9])/(1024.0*1024.0)
    #         temp = temp*8
    #         sine.append(round(temp,2))
    #         tempo.append(float(line[9])*1000)
    #     else:
    #         temp1 = int(line[9])/(1024.0 * 1024.0)
    #         temp1 = temp1 * 8
    #         sine1.append(round(temp1,2))
    # new_data = dict(x=tempo, y=sine, y1=sine1)
    # source.stream(new_data)
